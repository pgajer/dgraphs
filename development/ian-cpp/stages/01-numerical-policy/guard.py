...
def run(command,folder,study,wall_limit=900.,synthetic=False,max_attempts=1000):
    ...
    record=dict(command=list(map(str,command)),revision=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),cwd=os.getcwd(),wall_limit=wall_limit,expected_events=max_attempts,synthetic=synthetic,environment={k:v for k,v in env.items() if k.endswith('THREADS') or k=='PYTHONDONTWRITEBYTECODE'})
    ...
    try:
        with (folder/'stdout.log').open('wb') as stdout,(folder/'stderr.log').open('wb') as stderr,(folder/'samples.jsonl').open('w') as samples:
            ...
            while True:
                ...
                if elapsed-last>=.1:
                    ...
                    if events>max_attempts:reason=reason or 'single_solve_contract_exceeded'
                    if tree_bytes(folder)>2*2**30:reason=reason or 'run_output_limit'
                    if tree_bytes(study)>24*2**30:reason=reason or 'study_output_limit'
        ...
        if events>max_attempts:reason=reason or 'solve_contract_count'
