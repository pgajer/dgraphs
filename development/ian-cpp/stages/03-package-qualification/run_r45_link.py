...
root=Path(sys.argv[1]);out=root/'numerical-v3';out.mkdir(exist_ok=False)
...
prior=load(root/'numerical-v2/ledger.json');prior_processes=prior['prior_processes']+prior['processes'];assert prior['complete']
ledger=dict(prior_processes=prior_processes,revision=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),schedule=[],processes=[],checks={},complete=False)
for runtime in ['r45-v3']:
...
assert sum(c['entries'] for c in ledger['schedule'])==26
...
try:
 for item in ledger['schedule']:
  ...
  used=sum(p['optimizer_calls'] for p in ledger['prior_processes']+ledger['processes']);wall=sum(p['wall_seconds'] for p in ledger['prior_processes']+ledger['processes']);entries=sum(p['engine_entries'] for p in ledger['prior_processes']+ledger['processes'])
  assert used<2000 and wall<3600 and entries+item['entries']<=90 and shutil.disk_usage(root).free>20*2**30
  reserve(out/'reservations.json',90,dict(name=item['name'],engine_entries=item['entries'],command=item['command']))
