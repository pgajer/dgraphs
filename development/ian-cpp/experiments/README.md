...
Start with the [project aims](../docs/project-aims.md), [synthesis](synthesis.md) and [HTML catalogue](build/index.html). The catalogue contains 30 executed questions mapping to 29 independently accepted bounded milestones, plus five separate unexecuted proposals. All five authorized continuation stages are accepted; the scientific benchmark records mixed findings. Historical phases and evidence stay in place. `.yml` files use JSON syntax, valid YAML 1.2, so no YAML package is needed.
