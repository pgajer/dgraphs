...
out=root/'final-checks-v3';out.mkdir(exist_ok=False)
ledgers=[load(root/('numerical-v'+str(n))/'ledger.json') for n in [1,2,3]]
...
assert sum(p['engine_entries'] for p in processes)==90 and sum(p['optimizer_calls'] for p in processes)==835
files=[]
for num in [1,2,3]:
 for runtime in (['rdevel','r45'] if num==1 else ['rdevel-v2','r45-v2'] if num==2 else ['r45-v3']):
...
files.append(root/'r45-v3/result-checks-v3/strict-duplicates.jsonl')
...
assert counts['payloads']==649,counts
for runtime in ['rdevel','r45','rdevel-v2','r45-v2','r45-v3']:
 result=load(root/runtime/('result-checks-v3' if runtime.endswith('-v3') else 'result-checks-v2' if runtime.endswith('-v2') else 'result-checks-v1')/'summary.json');assert result['passed'];checks.append(result)
for runtime in ['rdevel-v2','r45-v3']:
 ...
 if runtime=='r45-v3':
  main=root/runtime/'library/dgraphs/libs/dgraphs.so';links=subprocess.check_output(['otool','-L',str(main)],text=True);(out/'r45-main-linkage.txt').write_text(links);assert '/Versions/4.7/' not in links and '/Versions/4.5-arm64/' in links
