test_that("minimal sKNN graphs preserve fitting fields without lifecycle repair", {
    ...
    expect_identical(graph$graph_detail, "minimal")
    expect_false(graph$lifecycle_branches)
    expect_equal(graph$raw_adj_list, graph$adj_list)
    expect_equal(graph$raw_weight_list, graph$weight_list)
    expect_equal(graph$pruned_adj_list, graph$adj_list)
    expect_equal(graph$pruned_weight_list, graph$weight_list)
    expect_null(graph$raw_repaired_adj_list)
    expect_equal(graph$n_components_before, 2L)
    expect_equal(graph$n_components_after, 2L)
})
...
test_that("minimal sKNN graphs reject lifecycle operations", {
    ...
    expect_error(
        create.sknn.graph(
            X, 1, graph.detail = "minimal", connect.components = TRUE
        ),
        "requires prune.method = 'none'"
    )
    expect_error(
        create.sknn.graph(
            X, 1, graph.detail = "minimal", prune.edges = TRUE
        ),
        "requires prune.method = 'none'"
    )
})
...
test_that("precomputed ordered neighbors reproduce scalar ANN construction", {
    ...
    direct <- create.sknn.graph(
        X, k, neighbor.method = "ann", graph.detail = "minimal"
    )
    supplied <- create.sknn.graph(
        X, k, neighbor.method = "ann", knn.index = cached,
        graph.detail = "minimal"
    )

    expect_equal(supplied, direct)
    expect_error(
        create.sknn.graph(X, k, neighbor.method = "exact", knn.index = cached),
        "supported only"
    )
    expect_error(
        create.sknn.graph(
            X, k, neighbor.method = "ann", knn.index = cached[-1, ]
        ),
...
test_that("create.sknn.graphs validates series-specific controls", {
    ...
    expect_error(
        create.sknn.graphs(X, k.values = c(1, 1)),
        "duplicate"
    )
    expect_error(
        create.sknn.graphs(X, k.values = 2, neighbor.method = "exact"),
        "requires neighbor.method = 'ann'"
    )
})
