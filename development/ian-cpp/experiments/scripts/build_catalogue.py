...
def main():
    ...
    documents=[p for p in md_sources() if 'build' not in p.relative_to(ROOT).parts]
    assert len({destination(p) for p in documents})==len(documents),'Colliding document destinations'
    for p in documents:
