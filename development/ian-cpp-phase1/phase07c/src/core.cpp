...
ErrorKind classify(const std::string& code) {
    ...
    if (code == "retry_exhausted" || code == "invalid_solver_result" || code == "solver_construction" || code == "invalid_affinity" ||
...
std::string fixed_probe(const std::string& text, Observer* observer) {
    ...
    require(input.contains("numerical_policy") && input.at("numerical_policy")==numerical_policy,"unsupported_policy");
