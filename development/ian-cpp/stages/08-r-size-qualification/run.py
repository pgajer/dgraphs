...
H=Path(__file__).resolve().parent;ROOT=H.parents[3];P=Path(sys.argv[1]);B=P/(sys.argv[2] if len(sys.argv)>2 else 'build-v1');O=P/(sys.argv[3] if len(sys.argv)>3 else 'panel-v1');O.mkdir(exist_ok=False)
...
prior=load(P/sys.argv[4]/'ledger.json') if len(sys.argv)>4 else None
L=dict(prior=(prior.get('prior',[])+prior['processes']) if prior else [],processes=[],cases=[],engine_entries=prior['engine_entries'] if prior else 0,complete=False,larger_gate=False);save=lambda:write(O/'ledger.json',L);save()
def execute(name,cmd):
 used=sum(x['optimizer_calls'] for x in L['prior']+L['processes']);wall=sum(x['wall_seconds'] for x in L['prior']+L['processes']);assert L['engine_entries']<80 and used+1500<=16000 and wall<7200 and shutil.disk_usage(P).free>20*2**30
 reserve(O/'reservations.json',80,dict(name=name,command=list(map(str,cmd))));L['engine_entries']+=1;save();folder=O/'runs'/name
