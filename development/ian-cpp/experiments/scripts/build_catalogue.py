...
def main():
    ...
    for p in md_sources():
        ...
        out=destination(p);ast,pandoc=document(p);html_document(p,out,ast,pandoc);pdf_document(ast,out.with_suffix('.pdf'),p.parent.name if p.name=='report.md' else p.stem,page_per_section=p.name=='meeting-figure-selection.md')
        rendered[str(p.relative_to(ROOT))]={'source_sha256':sha(p),'html':str(out.relative_to(ROOT)),'html_sha256':sha(out),'pdf':str(out.with_suffix('.pdf').relative_to(ROOT)),'pdf_sha256':sha(out.with_suffix('.pdf'))}
    ...
    for p,m in rec:
        ...
        cards[kind].append('<article data-group="'+m['programme_group']+'" data-execution="'+kind+'" data-audit="'+('accepted' if kind=='executed' else 'unexamined')+'" data-review="pending"><div class="meta">'+m['id']+' · Phase '+html.escape(', '.join(m['phase_aliases']))+' · '+groups[m['programme_group']]+'</div><h3>'+link(p/'build/report.html',m['title'])+'</h3><span class="badge '+kind+'">'+html.escape(m['execution_status'])+'</span><p>'+html.escape(status)+'. New summary: not independently reviewed.</p>'+link(p/'build/report.html',m['outcome'])+preview+'<p>'+link(p/'build/report.html','Full record')+' · '+link(p/'build/report.pdf','PDF')+'</p><p class="meta">Dependencies: '+deps+'</p></article>')
    controls='''<div class="controls"><label>Search <input id="search" type="search" placeholder="Question, phase, method…"></label><label>Topic <select id="group"><option value="">All topics</option>'''+''.join('<option value="'+k+'">'+v+'</option>' for k,v in groups.items())+'''</select></label><label>Execution <select id="execution"><option value="">All records</option><option value="executed">Executed</option><option value="proposal">Proposals</option></select></label><label>Historical audit <select id="audit"><option value="">All</option><option value="accepted">Bounded evidence accepted</option><option value="unexamined">Not examined</option></select></label><label>New presentation <select id="review"><option value="">All</option><option value="pending">Awaiting independent review</option></select></label></div><p id="count" aria-live="polite"></p>'''
    ...
    manifest={'operation':'full scientific rendering only; zero optimizer calls','built_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'git_head':subprocess.run(['git','rev-parse','HEAD'],cwd=ROOT,capture_output=True,text=True,check=True).stdout.strip(),'source_hashes':before,'rendered':rendered,'assembly':assembly,'book_sha256':sha(B/'experiment-book.pdf'),'index_sha256':sha(B/'index.html'),'figure_manifest_sha256':sha(B/'figure-manifest.json'),'counts':{'documented':22,'executed_questions':17,'historical_milestones':16,'unexecuted_proposals':5,'new_reports_independently_reviewed':0},'environment':{'python':platform.python_version(),'reportlab':reportlab.Version,'pypdf':pypdf.__version__,'pandoc':subprocess.run([pandoc,'--version'],capture_output=True,text=True).stdout.splitlines()[0]},'external_backup':'unknown','private_evidence_reads':'none; private paths remain links only'}
