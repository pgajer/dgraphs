...
The accepted [pruning diagnosis](032-helix-pruning-diagnosis/report.md) now locates the splits: ordinary local pruning deletes genuine bridges after each helix has become an accurate consecutive-point chain. Both interfaces agree; no numerical failure is involved at the first cuts. Any method change needs its own declared success criteria and additional confirmatory coordinate draws. The [project status](analysis-queue.md) keeps this research question separate from public API, scale, platform and application decisions.

The [public-interface qualification](035-public-interface/report.md) now tests explicit setup and exported defaults against saved results in two private Mac arm64 R installations. Independent fresh builds, package checks and 56-call replay pass; all findings and the exact report/PDF are accepted. The next research study will evaluate the scientific effects of preserving connections, separately from interface correctness.
