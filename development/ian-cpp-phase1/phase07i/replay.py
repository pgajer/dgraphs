"""One prespecified original-unit coordinate-range call, observing the backend without altering it."""
import time,warnings
from scipy.optimize import linprog
from scipy.optimize._highspy import _core as h
from support import *
f,out=map(Path,sys.argv[1:]);out.mkdir(parents=True,exist_ok=False);A,b,c=read_arrays(f);origin=load(f/'case.json');original_class=h._Highs;calls=0
