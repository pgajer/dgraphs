...
engine=w/'phase07e/build-v1/ian_engine';test=engine.with_name('ian_retry_tests');tool=engine.with_name('ian_checkpoint_tool')
fixtures={e['name']:e for e in load(w/'phase07e/engine-fixtures-v1/manifest.json')['cases']}
...
if True:
 ...
 check('checkpoint_policy_and_counter',payload['policy']=='IAN evaluated-LP retry units11 almost 0.1' and payload['solves']==s['solves'],payload_summary={k:payload[k] for k in ['policy','solves','iteration','source','configuration']})
