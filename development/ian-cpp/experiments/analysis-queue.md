...
### Stage 3 execution complete — independent review pending

Fresh package installations and optional backend builds preserve IAN results on this Mac. The full package check exposed mixed R-version linkage in the initial R 4.5.2 build environment; a separately declared correction rebuilds with the intended R library and repeats its complete panel. Across retained initial and corrected evidence, 90 R calls contain 835 solver attempts. Final R-devel checks have zero errors/warnings and two notes; corrected R 4.5.2 checks have zero errors, one upstream-header compiler warning and four notes. Examples, tests and vignettes pass in the corrected environment. The [report](029-package-qualification/report.md) records these boundaries and earlier failures. Stage 4 has not begun; it awaits independent acceptance and closure of findings.
