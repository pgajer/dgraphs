...
from support import *
H=HERE;w=Path(sys.argv[1]);root=Path(sys.argv[2]);root.mkdir(parents=True,exist_ok=False);build=w/'phase07e/build-v1';candidate=H/'candidate'
prior=load(w/'phase07e/diagnostic-v1/ledger.json');assert prior['complete'] and prior['regression_passed']
...
cases=load(w/'phase07e/engine-fixtures-v1/manifest.json')['cases'];cases.sort(key=lambda x:x['name']!='helix_500')
save();gate=True
try:
 for case in cases:
