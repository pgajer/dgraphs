...
{
  public:
    Interval() : m_start(0), m_d(0), m_pseudo_x(0), m_pseudo_y(0), m_min(0),
      m_next(nullptr), m_edge(nullptr), m_source_index(0), m_direction(UNDEFINED_DIRECTION) {}
    ~Interval() {}
    ...
    {
        GEODESIC_ASSERT(x >= 0.0 && x <= m_edge->length());
...
class IntervalList // list of the of intervals of the given edge
{
  public:
    ...
    {
        GEODESIC_ASSERT(offset >= 0.0 && offset <= m_edge->length());
...
class SurfacePointWithIndex : public SurfacePoint
{
  public:
    ...
    {
        GEODESIC_ASSERT(x->type() != UNDEFINED_POINT && y->type() != UNDEFINED_POINT);
