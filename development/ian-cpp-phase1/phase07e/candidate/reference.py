...
def make_reference(folder,condition,metadata):
 ...
 def solve(prob,x,site,u=None,cinfo=None,c=None):
  ...
  for attempt,tolerance in enumerate([1e-9,1e-11]):
   ...
   if not (np.isfinite(data['A'].data).all() and np.isfinite(rhs).all() and np.isfinite(data['c']).all()):raise audit.InvalidSolve('invalid_solver_coefficients')
   ...
   if len(raw.x)!=len(u) or len(raw.z)!=len(b):raise audit.InvalidSolve('invalid_solver_dimensions')
   ...
   usable=usable_return(scales,z,u,objective,len(u),len(b))
   ...
   accepted,eligible=classify_return(usable,str(raw.status),report['objective_relative_error'],
