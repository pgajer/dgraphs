local.extrema.chain <- function(detect.maxima = TRUE, min.neighborhood.size = 2) {
    detect.local.extrema(adj.list = list(2L, c(1L, 3L), 2L), length.list = list(1, c(1, 1), 1), y = c(1,
        3, 0), max.radius = 1, min.neighborhood.size = min.neighborhood.size, detect.maxima = detect.maxima)
}
...
test_that("empty extrema retain the requested maxima or minima type", {
    for (detect.maxima in c(TRUE, FALSE)) {
        ...
        expected.type <- if (detect.maxima)
            "Maximum"
        else "Minimum"
...
test_that("nonempty extrema remain compatible with legacy objects", {
    for (detect.maxima in c(TRUE, FALSE)) {
        ...
        expected.vertices <- if (detect.maxima)
            2L
        else c(1L, 3L)
        ...
        s <- summary(x)
        expect_identical(s$extrema_type, expected.type)
        expect_identical(s$extrema_details$fn_value, sort(x$values, decreasing = detect.maxima))
        x$detect.maxima <- NULL
...
test_that("the requested detection type must be known", {
    expect_error(local.extrema.chain(NA), "'detect.maxima' must be a scalar logical", fixed = TRUE)
})
