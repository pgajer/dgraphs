...
library(dgraphs)
backend <- normalizePath(args[2], mustWork=TRUE)
...
save <- function() saveRDS(list(calls=ledger, checks=checks,total_solves=total),file.path(out,"ledger.rds"))
...
run <- function(name, x, d=NULL, ids=NULL, participants=NULL, detail="summary", limit=80L, fault="none") {
 stopifnot(length(ledger)<30L, nrow(x)<=96L,total+limit<=600L)
