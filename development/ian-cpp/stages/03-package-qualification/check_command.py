...
child=root/'child.py';child.write_text('import json,sys,time\nfrom pathlib import Path\np=Path(sys.argv[1]);sentinel=p.parent/"active-child"\nwith sentinel.open("x") as lock:\n start=time.time_ns();time.sleep(0.2);p.write_text(json.dumps(dict(start=start,end=time.time_ns(),exclusive_sentinel=True)))\nsentinel.unlink()\nraise SystemExit(int(sys.argv[2]))\n')
...
assert not list(root.rglob('*.tmp')) and not (root/'active-child').exists()
assert all(i['exclusive_sentinel'] for i in intervals)
