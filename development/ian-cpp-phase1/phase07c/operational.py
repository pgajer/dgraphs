...
for label,data in [('missing-policy',{k:v for k,v in load(small).items() if k!='numerical_policy'}),('wrong-policy',dict(load(small),numerical_policy='IAN evaluated-LP 1.0'))]:
 ...
 for condition in ['native','evaluated']:
  name=label+'-'+condition;child=root/name/'child';cmd=[engine,f,child] if condition=='native' else [sys.executable,'-B',H/'reference.py',f,child,'evaluated']
...
if status['complete']:
 ...
 check('checkpoint_policy_and_counter',payload['policy']=='IAN evaluated-LP retry 0.1' and payload['solves']==s['solves'],payload_summary={k:payload[k] for k in ['policy','solves','iteration','source','configuration']})
 ...
 for label,key,value in [('policy','policy','IAN evaluated-LP 1.0'),('source','source','0'*64),('configuration','configuration','0'*64)]:
