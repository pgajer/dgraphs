...
struct vertex_path_info_t {
    std::vector<std::pair<int, int>> containing_paths;
    std::vector<int> position_in_path;
    ...
    get_full_paths(size_t desired_length,
        ...
        for (size_t i = 0; i < containing_paths.size(); ++i) {
            const auto& endpoints = containing_paths[i];
            auto path_it = shortest_paths.find({
                std::min(endpoints.first, endpoints.second),
                std::max(endpoints.first, endpoints.second)
            });
            if (path_it != shortest_paths.end() &&
                path_it->second.size() == desired_length) {
                result.emplace_back(path_it->second, position_in_path[i]);
            }
        }
        return result;
    }
...
struct path_graph_plm_t {
    ...
    std::vector<std::pair<int, int>> longest_paths;

    std::vector<std::pair<std::vector<int>, int>>
    get_paths_containing(int vertex, int desired_length) const {
        return vertex_paths[static_cast<size_t>(vertex)].get_full_paths(
            static_cast<size_t>(desired_length),
            shortest_paths
        );
    }
};
...
path_graph_plm_t create_path_graph_plm(
    ...
    for (int start = 0; start < n_vertices; ++start) {
        ...
        while (!q.empty()) {
            ...
            const double current_dist = distances[static_cast<size_t>(current)];
            const int next_hops = hops[static_cast<size_t>(current)] + 1;
            const auto& current_adj = adj_list[static_cast<size_t>(current)];
            const auto& current_weights = weight_list[static_cast<size_t>(current)];
        ...
        for (int v = 0; v < n_vertices; ++v) {
            if (v != start && hops[static_cast<size_t>(v)] <= h) {
                ...
                if (start < v) {
                    ...
                    std::reverse(path.begin(), path.end());
                    if (hops[static_cast<size_t>(v)] == h) {
    ...
    result.vertex_paths.resize(static_cast<size_t>(n_vertices));
    for (const auto& path_entry : result.shortest_paths) {
        ...
        for (size_t pos = 0; pos < path.size(); ++pos) {
            const int vertex = path[pos];
            result.vertex_paths[static_cast<size_t>(vertex)].containing_paths.push_back(endpoints);
            result.vertex_paths[static_cast<size_t>(vertex)].position_in_path.push_back(
                static_cast<int>(pos)
            );
        }
    }
...
SEXP path_graph_plm_to_R(const path_graph_plm_t& path_graph) {
    const int n_vertices = static_cast<int>(path_graph.adj_list.size());
    ...
    const int n_paths = static_cast<int>(path_graph.shortest_paths.size());
    SEXP shortest_paths = PROTECT(Rf_allocVector(VECSXP, 3));
    SEXP i_coords = PROTECT(Rf_allocVector(INTSXP, n_paths));
    SEXP j_coords = PROTECT(Rf_allocVector(INTSXP, n_paths));
    SEXP paths = PROTECT(Rf_allocVector(VECSXP, n_paths));
    int path_idx = 0;
    for (const auto& entry : path_graph.shortest_paths) {
        INTEGER(i_coords)[path_idx] = entry.first.first + 1;
        INTEGER(j_coords)[path_idx] = entry.first.second + 1;
        SEXP path_vec = PROTECT(Rf_allocVector(INTSXP, entry.second.size()));
        for (size_t m = 0; m < entry.second.size(); ++m) {
            INTEGER(path_vec)[m] = entry.second[m] + 1;
        }
        SET_VECTOR_ELT(paths, path_idx, path_vec);
        UNPROTECT(1);
        ++path_idx;
    }
    SET_VECTOR_ELT(shortest_paths, 0, i_coords);
    SET_VECTOR_ELT(shortest_paths, 1, j_coords);
    SET_VECTOR_ELT(shortest_paths, 2, paths);
    SEXP shortest_paths_names = PROTECT(Rf_allocVector(STRSXP, 3));
    SET_STRING_ELT(shortest_paths_names, 0, Rf_mkChar("i"));
    SET_STRING_ELT(shortest_paths_names, 1, Rf_mkChar("j"));
    SET_STRING_ELT(shortest_paths_names, 2, Rf_mkChar("paths"));
    Rf_setAttrib(shortest_paths, R_NamesSymbol, shortest_paths_names);
    SET_VECTOR_ELT(r_result, 3, shortest_paths);

    SEXP vertex_paths = PROTECT(Rf_allocVector(VECSXP, n_vertices));
    for (int v = 0; v < n_vertices; ++v) {
        const auto& vp = path_graph.vertex_paths[static_cast<size_t>(v)];
        const int n_vertex_paths = static_cast<int>(vp.containing_paths.size());
        SEXP path_matrix = PROTECT(Rf_allocMatrix(INTSXP, n_vertex_paths, 3));
        ...
        for (int i = 0; i < n_vertex_paths; ++i) {
            m[i] = vp.containing_paths[static_cast<size_t>(i)].first + 1;
            m[i + n_vertex_paths] =
                vp.containing_paths[static_cast<size_t>(i)].second + 1;
            m[i + 2 * n_vertex_paths] =
                vp.position_in_path[static_cast<size_t>(i)] + 1;
        }

        SET_VECTOR_ELT(vertex_paths, v, path_matrix);
        UNPROTECT(1);
    }
    SET_VECTOR_ELT(r_result, 4, vertex_paths);
...
SEXP S_create_path_graph_plm(SEXP s_adj_list, SEXP s_edge_length_list, SEXP s_h) {
    std::vector<std::vector<int>> adj_vect = convert_adj_list_from_R(s_adj_list);
    std::vector<std::vector<double>> weight_vect = convert_weight_list_from_R(s_edge_length_list);
    if (!Rf_isInteger(s_h) || Rf_length(s_h) < 1) {
        Rf_error("h must be an integer scalar.");
    }
    path_graph_plm_t path_graph = create_path_graph_plm(
        adj_vect,
        weight_vect,
        INTEGER(s_h)[0]
    );
    return path_graph_plm_to_R(path_graph);
}
