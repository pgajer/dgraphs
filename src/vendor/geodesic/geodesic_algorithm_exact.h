...
class GeodesicAlgorithmExact : public GeodesicAlgorithmBase
{
  public:
    ...
    ~GeodesicAlgorithmExact() override { clear(); }
    GeodesicAlgorithmExact(const GeodesicAlgorithmExact&) = delete;
    GeodesicAlgorithmExact& operator=(const GeodesicAlgorithmExact&) = delete;
    std::function<void()> poll = [](){};
    size_t max_intervals = 200000, max_propagations = 100000;
    size_t peak_intervals = 0;
    unsigned propagations() const { return m_iterations; }
  ...
  private:
    ...
    interval_pointer allocate_interval() {
        poll();
        if(m_owned.size()>=max_intervals)throw ResourceLimit("interval_limit");
        std::unique_ptr<Interval> p(new Interval());
        interval_pointer raw=p.get();m_owned.emplace(raw,std::move(p));
        peak_intervals=std::max(peak_intervals,m_owned.size());return raw;
    }
    void release_interval(interval_pointer p) { m_owned.erase(p); }
    std::map<interval_pointer,std::unique_ptr<Interval>> m_owned;
    ...
    unsigned m_iterations = 0;   // used for statistics
...
{
    GEODESIC_ASSERT(point.type() != UNDEFINED_POINT);
...
GeodesicAlgorithmExact::list_edges_visible_from_source(MeshElementBase* p,
                                                       std::vector<edge_pointer>& storage)
{
    GEODESIC_ASSERT(p->type() != UNDEFINED_POINT);
...
GeodesicAlgorithmExact::erase_from_queue(interval_pointer p)
{
    ...
    {
        GEODESIC_ASSERT(m_queue.count(p) <= 1); // the set is unique
...
GeodesicAlgorithmExact::intersect_intervals(
  ...
  IntervalWithStop* one) // intersecting two intervals with up to three intervals in the end
{
    GEODESIC_ASSERT(zero->edge()->id() == one->edge()->id());
    GEODESIC_ASSERT(zero->stop() > one->start() && zero->start() < one->stop());
    GEODESIC_ASSERT(one->min() < GEODESIC_INF / 10.0);
  ...
  std::vector<SurfacePoint>* stop_points)
{
    ...
    while (!m_queue.empty()) {
        poll();
        if(m_iterations>=max_propagations)throw ResourceLimit("propagation_limit");
        ...
        GEODESIC_ASSERT(min_interval->d() < GEODESIC_INF);
    ...
    /*	for(unsigned i=0; i<m_edge_interval_lists.size(); ++i)
            {
                    ...
                    GEODESIC_ASSERT(p->start() == 0.0);
                    while(p->next())
                    {
                            GEODESIC_ASSERT(p->stop() == p->next()->start());
                            GEODESIC_ASSERT(p->d() < GEODESIC_INF);
                            p = p->next();
                    }
            }*/
}
...
GeodesicAlgorithmExact::update_list_and_queue(list_pointer list,
                                              ...
                                              unsigned num_candidates)
{
    GEODESIC_ASSERT(num_candidates <= 2);
    // GEODESIC_ASSERT(list->first() != nullptr);
    ...
    if (list->first() == nullptr) {
        ...
        if (num_candidates == 1) {
            ...
            GEODESIC_ASSERT(first->stop() == second->start());
    ...
    for (unsigned i = 0; i < num_candidates; ++i) // for all new intervals
    {
               ...
               p->start() < q->stop() - local_epsilon) // go through all old intervals
        {
            ...
            {
                i_new[j] = allocate_interval(); // create new intervals
            }
...
GeodesicAlgorithmExact::compute_propagated_parameters(
  ...
  IntervalWithStop* candidates) // if it is the last interval on the edge
{
    GEODESIC_ASSERT(pseudo_y <= 0.0);
    GEODESIC_ASSERT(d < GEODESIC_INF / 10.0);
    GEODESIC_ASSERT(begin <= end);
    GEODESIC_ASSERT(first_interval ? (begin == 0.0) : true);
    ...
    p->pseudo_x() = cos_alpha * pseudo_x + sin_alpha * pseudo_y;
    p->pseudo_y() = -sin_alpha * pseudo_x + cos_alpha * pseudo_y;
    GEODESIC_ASSERT(p->pseudo_y() <= 0.0);
  ...
  interval_pointer source_interval) // up to two candidates
{
                 ...
                 // intervals
    {
        for (unsigned i = 0; i < num_candidates; ++i) {
            ...
            GEODESIC_ASSERT(p->start() < p->stop());
        }
    } else // now we have to invert the intervals
    {
        for (unsigned i = 0; i < num_candidates; ++i) {
            ...
            GEODESIC_ASSERT(p->start() < p->stop());
            GEODESIC_ASSERT(p->start() >= 0.0);
            GEODESIC_ASSERT(p->stop() <= edge->length());
        }
    }
}
                                            ...
                                            unsigned& best_source_index)
{
    ...
    if (point.type() == EDGE) {
        ...
        if (best_interval) {
            // GEODESIC_ASSERT(best_interval && best_interval->d() < GEODESIC_INF);
  ...
  std::vector<SurfacePoint>& path)
{
    ...
    if (best_interval) // if we did not hit the face source immediately
    {
                  ...
                  // inside the FACE, we obviously hit the source)
        {
            ...
            poll();
            if(path.size()>=65536)throw ResourceLimit("trace_point_limit");
            ...
            // geodesic::diagnostic_stream() << total_distance + length(path) << std::endl;
            GEODESIC_ASSERT(total_distance < GEODESIC_INF);
...
GeodesicAlgorithmExact::print_statistics() const
{
    ...
    geodesic::diagnostic_stream() << "uses about " << memory / 1e6 << "Mb of memory" << std::endl;
    geodesic::diagnostic_stream() << interval_counter << " total intervals, or " << intervals_per_edge
              << " intervals per edge" << std::endl;
    geodesic::diagnostic_stream() << "maximum interval queue size is " << m_queue_max_size << std::endl;
    geodesic::diagnostic_stream() << "number of interval propagations is " << m_iterations << std::endl;
}
