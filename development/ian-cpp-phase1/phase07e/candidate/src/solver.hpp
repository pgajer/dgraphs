...
inline LPResult solve_lp(const Mat &D, const Edges &edges, const Vec &u, double C,
                         bool parameterized, bool inject = false, double tolerance = 1e-9, bool damage = false) {
    ...
    auto disposition=classify_return(accepted,sol.status==ClarabelSolved,sol.status==ClarabelAlmostSolved,
