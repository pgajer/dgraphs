local({
  api <- list(solve = getFromNamespace("quadform_geodesics_solver", "dgraphs"))
...
test_that("the installed interface stays internal and never compiles at runtime", {
  expect_false("quadform_geodesics_solver" %in% getNamespaceExports("dgraphs"))
  expect_false(grepl("sourceCpp|QGC_BUILD_CACHE|reticulate", paste(deparse(body(api$solve)),collapse = "")))
  expect_true("rcpp_quadform_geodesics_solver" %in% names(asNamespace("dgraphs")))
})
})
