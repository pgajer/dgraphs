...
### Stage 2 execution — audit pending

The six remaining 1,000-profile examples all complete with native/Python agreement: cloud, separated lobes and intrinsic-dimension 2–5 quadforms. All 388 attempts pass without retry. Saved-product checks localize Python warnings to dense vector matrix multiplication; all corresponding values remain finite and agree with scalar sums. The low-level cause is unresolved. See the [stage-2 report](028-scale-panel/report.md). Stage 3 remains paused until the independent audit and any corrections are complete.
