...
Mesh::initialize_mesh_data(
  ...
  const Faces& tri) // build mesh from regular point-triangle representation
{
    GEODESIC_ASSERT(p.size() % 3 == 0);
    unsigned const num_vertices = p.size() / 3;
    GEODESIC_ASSERT(tri.size() % 3 == 0);
    ...
    for (unsigned i = 0; i < num_faces; ++i) // copy adjacent vertices to polygons/faces
    {
        ...
        for (unsigned j = 0; j < 3; ++j) {
            ...
            GEODESIC_ASSERT(vertex_index < num_vertices);
...
Mesh::build_adjacencies()
{
    ...
    for (unsigned i = 0; i < m_faces.size(); ++i) {
        ...
        for (unsigned j = 0; j < 3; ++j) {
            ...
            GEODESIC_ASSERT(vertex_id < m_vertices.size());
            count[vertex_id]++;
        }
    }
    ...
    for (unsigned i = 1; i < half_edges.size(); ++i) {
        if (half_edges[i] != half_edges[i - 1]) {
            ...
            {                          // if it fails, most likely the input data are messed up
                GEODESIC_ASSERT(half_edges[i] != half_edges[i + 1]);
            }
        }
    }
    ...
    for (unsigned i = 0; i < half_edges.size();) {
        ...
        GEODESIC_ASSERT(e.length() > 1e-100); // algorithm works well with non-degenerate meshes only
    ...
    for (unsigned i = 0; i < m_edges.size(); ++i) {
        Edge& e = m_edges[i];
        GEODESIC_ASSERT(e.adjacent_vertices().size() == 2);
    ...
    auto cos_from_edges = [](double const a, double const b, double const c) -> double {
        GEODESIC_ASSERT(a > 1e-50);
        GEODESIC_ASSERT(b > 1e-50);
        GEODESIC_ASSERT(c > 1e-50);
    ...
    for (unsigned i = 0; i < m_faces.size(); ++i) {
        ...
        for (unsigned j = 0; j < 3; ++j) // compute angle adjacent to the vertex j
        {
            ...
            GEODESIC_ASSERT(angle > 1e-5); // algorithm works well with non-degenerate meshes only

            f.corner_angles()[j] = angle;
            sum += angle;
        }
        GEODESIC_ASSERT(std::abs(sum - M_PI) < 1e-5); // algorithm works well with non-degenerate meshes only
    }
...
Mesh::verify() // verifies connectivity of the mesh and prints some debug info
{
    ...
    GEODESIC_ASSERT(std::find(map.begin(), map.end(), false) == map.end());
    ...
    geodesic::diagnostic_stream() << "mesh has " << m_vertices.size() << " vertices, " << m_faces.size() << " faces, "
              << m_edges.size() << " edges\n";
    ...
    geodesic::diagnostic_stream() << total_boundary_edges << " edges are boundary edges\n";
    geodesic::diagnostic_stream() << "shortest/longest edges are " << shortest_edge << "/" << longest_edge << " = "
    ...
    geodesic::diagnostic_stream() << "enclosing XYZ box:"
              << " X[" << minx << "," << maxx << "]"
              << " Y[" << miny << "," << maxy << "]"
    ...
    double dy = maxy - miny;
    double dz = maxz - minz;
    geodesic::diagnostic_stream() << "approximate diameter of the mesh is " << std::sqrt(dx * dx + dy * dy + dz * dz)
              << std::endl;
    ...
    geodesic::diagnostic_stream() << "min/max face angles are " << min_angle / M_PI * 180.0 << "/"
