  ...
  public:
    ...
    face_pointer opposite_face(face_pointer f)
    {
        if (adjacent_faces().size() == 1) {
            GEODESIC_ASSERT(adjacent_faces()[0]->id() == f->id());
            return nullptr;
        }

        GEODESIC_ASSERT(adjacent_faces()[0]->id() == f->id() || adjacent_faces()[1]->id() == f->id());
