...
save <- function() saveRDS(list(calls=ledger, checks=checks,total_solves=total,prior_calls=prior_calls,prior_solves=prior_solves,backend_discovery=if(is.null(backend))"installed default" else backend),file.path(out,"ledger.rds"))
