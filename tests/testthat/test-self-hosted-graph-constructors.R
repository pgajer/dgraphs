...
test_that("DG6f lifecycle branches and MST repair metadata are self-hosted", {
    ...
    expect_true(all(lifecycle.fields %in% names(graph)))

    for (prefix in c("", "raw_", "pruned_", "raw_repaired_",
                    "pruned_repaired_", "repaired_pruned_")) {
        .dg6f.expect.weighted.adjacency(
            graph[[paste0(prefix, "adj_list")]],
            graph[[paste0(prefix, "weight_list")]],
            nrow(X)
        )
    }

    expect_equal(graph$n_components_raw, 2L)
    expect_equal(graph$n_components_raw_repaired, 1L)
    expect_equal(graph$n_components_pruned, 2L)
    expect_equal(graph$n_components_pruned_repaired, 1L)
    expect_equal(graph$n_components_repaired_pruned, 1L)
    expect_equal(graph$n_mst_edges_added, 1L)
    expect_equal(nrow(graph$mst_edge_matrix), 1L)
    expect_equal(length(graph$mst_edge_weight), 1L)

    expect_equal(graph$n_edges_in_raw_graph,
                 .dg6f.edge.count(graph$raw_adj_list))
    expect_equal(graph$n_edges_in_raw_repaired_graph,
                 .dg6f.edge.count(graph$raw_repaired_adj_list))
    expect_equal(graph$n_edges_in_pruned_repaired_graph,
                 .dg6f.edge.count(graph$pruned_repaired_adj_list))
    expect_equal(graph$n_edges_in_repaired_pruned_graph,
                 .dg6f.edge.count(graph$repaired_pruned_adj_list))
...
test_that("DG1 pure-R graph constructors are self-hosted", {
    ...
    expect_equal(length(joined), 3)
    expect_equal(joined[[2]], c(1L, 3L))
    expect_equal(joined[[3]], 2L)
...
test_that("DG1 graph utilities run without native gflow calls", {
    ...
    expect_equal(graph.connected.components(adj), c(1L, 1L, 1L, 1L, 5L))
