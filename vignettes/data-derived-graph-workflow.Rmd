...
For a task-oriented API catalog, start with [Finding your way around dgraphs](function-guide.html).
For reproducible input point clouds, see [Synthetic geometry and point sampling](synthetic-geometry.html).
...
minimum-spanning-tree completion. The full constructor results used here retain graph lifecycle stages
...
Each final graph exposes aligned adjacency and lengths through
`graph.adjacency()` and `graph.lengths()`. The
...
graph.summary <- do.call(rbind, lapply(names(graphs), function(name) {
    graph <- graphs[[name]]
    data.frame(graph = name, edges = sum(lengths(graph.adjacency(graph)))/2, raw.components = length(unique(graph.connected.components(graph))),
        final.components = length(unique(graph.connected.components(graph))), bridges = graph$metadata$n_mst_edges_added)
}))
graph.summary
```
...
edge.matrix <- convert.adjacency.to.edge.matrix(graph.adjacency(selected))$edge.matrix
