...
## Derived results and evidence snapshot

After all 18 jobs finish, commit the analysis sources and run the following from
that committed tree. These commands perform no new optimization. The first
recomputes every measured and diagnostic primal/dual check from raw vectors and
original NPZ coefficients, then creates the tables. The second verifies coverage,
source provenance and preservation of prior evidence, and hashes the bundle.
The summarizer defaults to the submitted layout. For new measurements, pass
`--fixtures`, `--diagnostics` and `--measured` with the reproduction directories.
The final snapshot tool deliberately checks the submitted revision and layout;
it is an evidence gate for this submission, not a general benchmark utility.

```sh
"$IAN_PY" "$IAN_PHASE/summarize.py" \
  --root "$IAN_WORKER/phase02" --output "$IAN_WORKER/phase02/analysis-v1"
"$IAN_PY" "$IAN_PHASE/finalize_evidence.py" \
  --root "$IAN_WORKER/phase02" --analysis "$IAN_WORKER/phase02/analysis-v1" --label v1
```

`REPORT.md` and `FIDELITY.md` are authored interpretation. The numerical tables
and `results.json` under `analysis-v1` are generated by `summarize.py`. The final
manifest excludes itself and the handoff, which is written afterward outside the
phase 2 root. Do not overwrite an existing evidence snapshot.
