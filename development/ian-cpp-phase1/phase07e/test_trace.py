...
check('wrong-raw-status',lambda d:d[first].update(solver_status='AlmostSolved'))
...
def third(d):
 e=copy.deepcopy(d[retry]);e.update(number=e['number']+1,attempt=2);d.insert(retry+1,e)
