...
checks=[];counts={};cpout=root/'checkpoint-checks';cpout.mkdir(exist_ok=False)
...
files={str(p):sha(p) for d in ['fixtures-v1','panel-v1','checkpoint-checks'] for p in (root/d).rglob('*') if p.is_file()}
