...
def make_reference(folder,condition,metadata):
 ...
 def solve(prob,x,site,u=None,cinfo=None,c=None):
  ...
  scales=np.array(raw.x[:len(u)]);x.value=scales;z=np.array(raw.z[:len(b)])
  ...
  state.C=C;state.emit(dict(event='solve',number=state.solves,site=site,C=C,A_data=A.data,A_indices=A.indices,A_indptr=A.indptr,A_shape=A.shape,b=b,c=c,upper=u,active=active.astype(int),scales=scales,dual=z,status=prob.status,objective=raw.obj_val,iterations=raw.iterations,seconds=time.perf_counter()-t,accepted=report['accepted'],validation=report,dual_check=dual,canonical_shape=data['A'].shape,canonical_soc=data['dims'].soc))
  state.solves+=1
  if not report['accepted']:raise audit.InvalidSolve(str(report['problems']))
  if condition=='evaluated' and max(dual.values())>1e-7:raise audit.InvalidSolve('dual_invalid')
