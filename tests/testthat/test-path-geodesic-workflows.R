...
test_that("DG6c geodesic distance helpers run self-hosted", {
    ...
    expect_equal(gnn$nn.index[1, ], c(2L, 3L))
    expect_equal(gnn$nn.index[4, ], c(3L, 2L))
...
test_that("DG6c native-backed PLM graph helper runs self-hosted", {
    ...
    expect_named(plm, c("graph", "h", "shortest.paths", "vertex.paths"))
    expect_equal(graph.adjacency(plm$graph)[[1]], c(2L, 3L))
    expect_equal(graph.lengths(plm$graph)[[1]], c(1, 3))
    expect_equal(graph.edge.attribute(plm$graph, "hops")[[1]], c(1L, 2L))
    expect_equal(plm$shortest.paths$paths[[2]], c(1L, 2L, 3L))
    expect_equal(plm$vertex.paths[[2]][, "position"], c(2L, 2L, 1L))
    expect_error(dgraphs::create.plm.graph(adj, weights, h = 2), "'h' must be odd")
})

test_that("DG6c graph core endpoints run self-hosted", {
    adj <- list(c(2L), c(1L, 3L, 4L), c(2L), c(2L))
    weights <- list(c(1), c(1, 2, 3), c(2), c(3))
    endpoints <- dgraphs::geodesic.core.endpoints(adj, weights, core.quantile = 0.5, endpoint.quantile = 0,
        use.approx.eccentricity = FALSE, verbose = FALSE)
